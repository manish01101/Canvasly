...
import { isPointInShape } from "../draw/utils";
...
export class CanvasEngine {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    // load from the localstorage on init
  ...
  private saveToStorage() {
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem("local_shapes", JSON.stringify(this.shapes));
    }
  }

  private loadFromStorage() {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("local_shapes");
      if (saved) {
        try {
